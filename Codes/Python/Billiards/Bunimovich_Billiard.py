...
def Bunimovich_billiard(p, v, n_col, L, R):
    point_list = [[p[0], p[1], 0]]
    for i in range(n_col):
        times = [line_intersection(p, v, np.array([0, 1, R]), np.array([-L / 2, R]), np.array([L / 2, R])),
                 line_intersection(p, v, np.array([0, -1, R]), np.array([-L / 2, -R]), np.array([L / 2, -R])),
                 circle_intersection(p, v, np.array([-L / 2, 0]), R, PI / 2, PI, 1),
                 circle_intersection(p, v, np.array([L / 2, 0]), R, -PI / 2, PI, 1)]
        ...
        if hit_wall == 0:
            ...
            v = bounce_circle(v, new_p, np.array([-L / 2, 0]))
        else:
            v = bounce_circle(v, new_p, np.array([L / 2, 0]))
        p = new_p
...
class Draw_single(Scene):
    def construct(self):
        ...
        " Bunimovich stadium geometrical elements"
        L = 5
        Radius = 2.2
        boundary_color = BLUE
        boundary_width = 6
        ...
        render1 = Bunimovich_billiard(p1, v1, num_col, L, Radius)
        self.add(b1)
        # print(render1)
        ...
        # render2 = Bunimovich_billiard(p2, v2, num_col, L, Radius)
        # self.add(b2)
        # print(render2)
        ...
        # render3 = Bunimovich_billiard(p3, v3, num_col, L, Radius)
        # self.add(b3)
        # print(render3)
        ...
        ###################################################
        " plotting boundary "
        self.add(Line([-L / 2, Radius, 0], [L / 2, Radius, 0], stroke_width=boundary_width, color=boundary_color))
        self.add(Line([-L / 2, -Radius, 0], [L / 2, -Radius, 0], stroke_width=boundary_width, color=boundary_color))
        self.add(Arc(Radius, 3 * PI / 2, -PI, arc_center=[-L / 2, 0, 0], stroke_width=boundary_width, color=boundary_color))
        self.add(Arc(Radius, PI / 2, -PI, arc_center=[L / 2, 0, 0], stroke_width=boundary_width, color=boundary_color))
...
class Draw_multiple(Scene):
    def construct(self):
        ...
        for l in range(Num_points):
            List.append(Bunimovich_billiard(p_list[l], v_list[l], num_col, L, Radius))
